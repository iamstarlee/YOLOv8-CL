...
class ReGhos_Block(nn.Module):
    def __init__(self, block, kernel_size=3):
        ...
        self.in_channels = self.original_block.in_channels
        self.kernel_size = self.original_block.kernel_size[0] if isinstance(self.original_block.kernel_size, tuple) else self.original_block.kernel_size
        self.stride = self.original_block.stride[0] if isinstance(self.original_block.stride, tuple) else self.original_block.stride
        self.padding = self.original_block.padding[0] if isinstance(self.original_block.padding, tuple) else self.original_block.padding
        # self.padding = kernel_size // 2 if self.padding == 0 else self.padding
        self.dilation = self.original_block.dilation[0] if isinstance(self.original_block.dilation, tuple) else self.original_block.dilation
        self.groups = self.original_block.groups if hasattr(self.original_block, 'groups') else 1
        self.weight = self.original_block.weight if hasattr(self.original_block, 'weight') else None
        self.bias = self.original_block.bias if hasattr(self.original_block, 'bias') else None
