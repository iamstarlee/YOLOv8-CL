...
from utils.utils_fit import fit_one_epoch_KL
...
if __name__ == "__main__":
    ...
    renamed_dict = {
                    ...
                    for k, v in tea_pretrained_dict.items()}


    load_key, no_load_key, temp_dict = [], [], {}
    for k, v in renamed_dict.items():  # 是否只加载骨干的参数，可以替换 renamed_dict
        if k in tea_model_dict.keys() and np.shape(tea_model_dict[k]) == np.shape(v):
            temp_dict[k] = v
            load_key.append(k)
        else:
    ...
    if local_rank == 0:
        ...
        loss_history    = LossHistory(log_dir, stu_model, input_shape=input_shape)
    else:
        loss_history    = None
    ...
    if Cuda:
        if distributed:
            ...
            model_train = torch.nn.DataParallel(stu_model)
    ...
    if True:
        ...
        #---------------------------------------#
        pg0, pg1, pg2 = [], [], []  
        for k, v in stu_model.named_modules():
        ...
        #----------------------#
        if local_rank == 0:
            eval_callback   = EvalCallback(stu_model, input_shape, class_names, num_classes, val_lines, log_dir, Cuda, \
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch_KL(model_train, stu_model, tea_model, ema, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, fp16, scaler, save_period, save_dir, local_rank)

            if distributed:
                dist.barrier()
        end_time = time.time()
        print(f"Duration is {(end_time - start_time):.2f} seconds.")
        if local_rank == 0:
