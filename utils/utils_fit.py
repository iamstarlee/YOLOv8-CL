...
def fit_one_epoch_KL(model_train, stu_model, teacher_model, ema, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    loss        = 0
    val_loss    = 0
    ...
    for iteration, batch in enumerate(gen):
        ...
        if not fp16:
            ...
            #     F.softmax(outputs_teacher[1] / temp, dim = -1)
            # )
            # distill_loss3 = soft_loss(
            #     F.softmax(outputs[3], dim = -1) ,
            #     F.softmax(outputs_teacher[3], dim = -1)
            # )
            # distill_loss4 = soft_loss(
    ...
    if local_rank == 0:
        ...
        if ema:
            ...
            save_state_dict = stu_model.state_dict()
